...
def _plot_to_image(probs: dict, w=400, h=250):
    try:
        fig = go.Figure(data=[go.Bar(
            x=list(probs.keys()), y=list(probs.values()),
            marker_color=["#ef4444", "#22c55e", "#f59e0b", "#3b82f6"],
        )])
        fig.update_layout(title="Probabilidades", xaxis_title="Classe",
                          yaxis_title="Probabilidade", margin=dict(l=20, r=20, t=40, b=30))
...
def generate_pdf(input_data: dict, trend: str, probs: dict, simulations: list, recommendations: dict) -> bytes:
    ...
    c.drawString(30, y, "Relatório FYP Lab")
    y -= 40
    ...
    c.setFont("Helvetica-Bold", 12)
    c.drawString(30, y, "Previsão do Modelo")
    c.setFont("Helvetica", 10)
    y -= 16
    c.drawString(40, y, f"Tendência: {trend}")
    y -= 40
    ...
    c.setFont("Helvetica-Bold", 12)
    c.drawString(30, y, "Cenários Simulados")
    c.setFont("Helvetica", 10)
    y -= 16
    for s in simulations:
        text = f"{s['name']}: {s.get('class', '')} | Deltas: {s.get('deltas', {})}"
        c.drawString(40, y, text[:90])
        y -= 14
    y -= 10

    c.setFont("Helvetica-Bold", 12)
    c.drawString(30, y, "Recomendações da IA")
    c.setFont("Helvetica", 10)
    y -= 16
